...
class CombatAnimations(Menu[None], ABC):
    ...
    def _apply_officer_battle_sprite(
        ...
        """Use the police officer battle sheet for officer NPCs."""
        officer_ids = {
            "knight",
            "officer",
            "omnigrunt",
        ...
        npc_ids = {
            ...
            getattr(template, "sprite_name", None),
            getattr(template, "combat_sheet", None),
        }

        # Officer Omoplata is stored internally as the agnidon monster.
        is_officer_omoplata = getattr(monster, "slug", None) == "agnidon"

        if is_officer_omoplata:
            monster.name = "Officer Omoplata"

        if officer_ids.intersection(npc_ids) or is_officer_omoplata:
            monster.sprite_config.sheet_path = (
                "gfx/sprites/battle/police-officer"
            )

    def _apply_arena_boss_battle_sprite(
        self,
        npc: NPC,
        monster: Monster,
    ) -> None:
        """Use the stripped white-belt sprite for Taba arena bosses."""
        arena_boss_npcs = {
            "acolyte",
            "garth",
            "cam",
        ...
        # Cam uses the lambert monster internally. Checking both identities
        # prevents another template or owner value from restoring its old sheet.
        is_cam = (
            npc.slug == "cam"
            or getattr(monster, "slug", None) == "lambert"
        )

        if npc.slug in arena_boss_npcs or is_cam:
            monster.sprite_config.sheet_path = (
                "gfx/sprites/battle/blackgi-whitebelt-stripped"
            )

    def _apply_selected_player_battle_sprite(
        self,
        monster: Monster,
    ) -> None:
    ...
    def animate_monster_release(
        ...
        if npc.slug == "cam":
            self._apply_arena_boss_battle_sprite(npc, monster)
        elif npc == self.session.player:
            self._apply_selected_player_battle_sprite(monster)
        else:
            self._apply_arena_boss_battle_sprite(npc, monster)
            self._apply_officer_battle_sprite(npc, monster)
    ...
    def animate_parties_in(self) -> None:
        ...
        self._apply_arena_boss_battle_sprite(opponent, opp_mon)
        self._apply_officer_battle_sprite(opponent, opp_mon)
        ...
        # Use Cam's arena sprite when Cam occupies the player-facing side.
        if player.slug == "cam":
            self._apply_arena_boss_battle_sprite(player, player_mon)
        else:
            self._apply_selected_player_battle_sprite(player_mon)
